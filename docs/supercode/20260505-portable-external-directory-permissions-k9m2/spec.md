...
## Objective
Make Supercode's checked-in agent permission defaults explicit for portable external-directory usage by ensuring the `executor` can access cross-worktree workflow paths without local path allowlists, while preserving `.env` read protection and keeping research-agent read defaults explicit.
...
- Built-in `executor` currently allows editing and bounded research delegation but does not explicitly define `external_directory` or `read`, which is the main friction point when execution touches root/worktree/docs boundaries.
...
- Ensure the built-in `executor` default permissions explicitly include `external_directory: "allow"` plus a nested read baseline that allows normal reads while denying `.env` and `.env.*` and allowing `.env.example`.
...
- Update built-in `executor` agent default permissions to include `external_directory: "allow"` and the same explicit nested `read` rules.
- Add or update tests that verify the built-in executor and research-agent read rules include the explicit read baseline and preserve `.env` protection.
...
- Granting new edit/write/bash permissions beyond executor's existing edit permission.
...
- Do not weaken existing agent write-deny or no-recursive-delegation policies; executor remains limited to its existing edit permission and bounded `task` delegation to `explorer`/`librarian`.
...
## Success Criteria
- `executor` built-in permission defaults include `external_directory: "allow"` and explicit `read["*"] = "allow"` while preserving `.env` denies and `.env.example` allow.
...
- No new permissions beyond executor external-directory allow and explicit read baseline are introduced.
...
- Revised scope after user clarified the primary target is `executor`; executor now explicitly receives portable external-directory allow and `.env`-protected read defaults.
