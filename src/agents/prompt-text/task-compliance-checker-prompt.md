...
If repository or external evidence is needed to judge the task, use `orchestrator-mediated-research`.

Use it when:
- file ownership is unclear
- task wording depends on current repository structure
- external dependency behavior affects whether the task is actually executable
