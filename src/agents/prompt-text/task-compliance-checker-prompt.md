...
Start from a fresh-session default: judge only the current artifacts, current evidence, current plan, and assigned task. Do not reuse stale or prior conclusions as proof.
...
8. Known exact paths and provided artifacts may be inspected directly; if additional discovery or external evidence is needed beyond provided context, use bounded `research-delegation`.
...
Put the verdict first. Keep blockers concise: at most 5 bullets unless more are required to justify `BLOCKED`.
